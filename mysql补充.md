...
具体来说，当有一条记录需要更新的时候，innodb存储引擎会先把记录写到redo log，并更新内存，这个时候更新就算完成了，同时innodb引擎会在适当的时候将这些操作落盘
...
1.当redo满了后，这时候系统会阻塞所有更新操作，把checkpoint往前推，redo留出空间可以继续写，checkpoint往前推进的过程对应的是原点与推进后的点之间的空间的日志刷盘，之后从write pos到checkpoint点之间的空间都是可以再次写入，这块为了好理解可以把整个日志的内存空间抽象成一个闭环，checkpoint之前到writepos这个弧上的空间就是未使用空间

2.系统内存不足时，当需要在内存中分配新的内存，内存不够用了，就要淘汰一些脏页数据，空出一些内存给别的数据页来使用，如果淘汰的是脏页，就要将脏页写到磁盘
