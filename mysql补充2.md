...
drop:如果这个表在独立表空间，实际上也只有讨论独立表空间的必要，那么会直接删除这个文件，如果大表drop，将会在瞬时产生大量的磁盘IO消耗，可能会造成整体mysql的卡顿，应该尽量避免线上的drop大表，这是磁盘文件层面的删除，还有有关这个表在bufferpool中的清除操作，mysql会将这个表在bufferpool中对应的页面都做一个删除，当然这个过程还需要给bufferpool上一个全局排它锁，实际上尽量少使用drop删表

truncate：截断表或者清空表，只删除表中的数据，但表的结构，索引，约束保持不变，在下次插入时自增长主键又从1开始

delete：delete 不带where子句效果和truncate一样，但是有一个非常重要的对比点，delete是DML语句，truncate和drop是DDL语句，所有DDL在执行以后都不可以回滚，同时delete不会重置表的自增主键
...
#### 重点！！ 对于快照读，RR模式下通过mvcc解决了快照读下的幻读问题，对于当前读，如果没有gaplock，只有recordlock那么是无法解决幻读的，gap lock的出现解决了这个问题
...
优化1：索引上的等值查询，给唯一索引加锁的时候，nkl退化为recordlock
